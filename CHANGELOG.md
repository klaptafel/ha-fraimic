...
- Device-identity fields (`identifiers`/`name`/`manufacturer`/`configuration_url`) shared by `__init__.py`'s `device_reg.async_get_or_create()` and `entity.py`'s `device_info()` consolidated into one `DeviceIdentity` dataclass, so the two can no longer drift apart. No behavior change.
